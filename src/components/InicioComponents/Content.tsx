        ...
        async function loadDefaultImage() {
            try {
                const response = await fetch(
                    "/Img/logo scalarbee/AbejaNegra.svg"
                );
                ...
                <div
                    ...
                    <h1
                        className={`font-bold text-[var(--texts)] 
                          text-3xl md:text-3xl xl:text-5xl 2xl:text-6xl
                          text-left  `}>
                        Donde la reparación se convierte en negocio.
                    </h1>
                    <RotatingText
                        texts={["Controla", "Administra", "Ordena", "Gestiona"]}
                        mainClassName="text-xl xl:text-3xl p-4 sm:px-2 font-bold md:px-3 bg-[var(--primary-dark)] text-black overflow-hidden py-0.5 sm:py-1 md:py-2 justify-center rounded-lg"
                        staggerFrom={"last"}
                        initial={{ y: "100%" }}
                        animate={{ y: 0 }}
                        exit={{ y: "-120%" }}
